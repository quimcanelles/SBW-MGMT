intensity.defoliation = function(outbreak, params, tbls){
  ...
  for(ispp in list.spp){
    ...
    aux = filter(outbreak, spp==ispp) %>%  #data.frrame(cell.id=vuln$cell.id, potential.spp=ispp, temp=vuln$temp, prec=vuln$prec, soil=vuln$soil.type) %>%
      ...
      dplyr::select(cell.id, spp, site.qual)
    dta = rbind(dta, aux)
  }
  dta = dta[-1,]
  aux = filter(outbreak, spp=="NonFor") %>% mutate(site.qual=0) %>% dplyr::select(cell.id, spp, site.qual)
  dta = rbind(dta, aux) 
  ...
  a = cut(outbreak$x, breaks=c(0,0.085,0.25,0.5,1), include.lowest=TRUE, right=TRUE, labels=levels) #QC edit. Original version breaks=c(0,0.005,0.25,0.5,1)
